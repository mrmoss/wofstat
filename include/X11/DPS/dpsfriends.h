#pragma ident	"@(#)dpsfriends.h	35.3	01/19/95 SMI"
/*
...
typedef struct _t_DPSSpaceRec {
  struct _t_DPSSpaceProcsRec *procs;
...
typedef void (*DPSSpaceProc)(DPSSpace space);
...
typedef void (*DPSContextProc)(DPSContext ctxt);

typedef void (*DPSContextBufProc)(DPSContext ctxt, char *buf,
				  unsigned int count);

typedef void (*DPSContextTypedArrayProc)(DPSContext ctxt,
					 DPSDefinedType type,
					 char *array, unsigned int length);

typedef void (*DPSWriteNumStringProc)(DPSContext ctxt,
				      DPSDefinedType type,
				      void *array,
				      unsigned int count, int scale);
