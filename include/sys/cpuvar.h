...
#pragma ident	"@(#)cpuvar.h	1.131	11/10/24 SMI"
...
typedef struct cpu {
	...
	struct cpu_pg	*cpu_pg;		/* cpu's prcoessor groups */
	int		cpu_pad0;		/* unused padding */
	...
	void		*cpu_reserved[5];	/* reserved for future use */

	struct cpu_physid *cpu_physid;		/* physical id cache */
	 ...
	 * the CPU so we can increment/decrement accordingly. Note that we
	 * store the negative value of the utilization to easily apply it
	 * to the PG hierarchy.
	 */
	int cpu_initial_util;
...
#define	CPUSET_NEXT(set, start, stop)	bt_getlowbit((set).cpub, start, stop)
...
#define	CPUSET_NEXT(set, start, stop)	bt_getlowbit(&(set), start, stop)
...
#define	BOOT_MAX_NCPUS	((boot_max_ncpus == -1) ? max_ncpus : boot_max_ncpus)
