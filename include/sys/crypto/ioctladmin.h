...
#pragma ident	"@(#)ioctladmin.h	1.1	03/06/11 SMI"
...
typedef struct crypto_get_dev_permitted {
	uint_t			dp_return_value;
	char			dp_dev_name[MAXNAMELEN];
	uint_t			dp_dev_instance;
	uint_t			dp_count;
	crypto_mech_name_t	dp_list[1];
} crypto_get_dev_permitted_t;

typedef struct crypto_get_soft_permitted {
	uint_t			sp_return_value;
	char			sp_name[MAXNAMELEN];
	uint_t			sp_count;
	crypto_mech_name_t	sp_list[1];
} crypto_get_soft_permitted_t;
...
#define	CRYPTO_GET_DEV_PERMITTED	CRYPTOADMIN(6)
#define	CRYPTO_GET_SOFT_PERMITTED	CRYPTOADMIN(7)
