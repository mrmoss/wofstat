...
#pragma ident	"@(#)ddi_impldefs.h	1.128	12/08/01 SMI"
...
struct dev_info  {
	...
	/* The following two members are owned by hotplug framework */
	/* hotplug handle list for child connections */
	struct ddi_hp_cn_handle *devi_hp_hdlp;
	/* reverse pointer to hotplug handle linked in parent devi_hp_hdlp */
	struct ddi_hp_cn_handle *devi_hp_parent_hdlp;

	struct in_node *devi_in_node; /* pointer to devinfo nodes's in_node_t */
};
...
#define	DEVI_NOATTACH		0x00008000 /* Do not attach the device */
#define	DEVI_PCI_DEVICE		0x00000100 /* dip is PCI */
...
#define	DEVI_DONT_ATTACH(dip)	(DEVI(dip)->devi_flags & DEVI_NOATTACH)
...
/*
 * Perhaps should be #if __sparc / #elif __i386 / #else err
