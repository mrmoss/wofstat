...
#pragma ident	"@(#)dtrace_impl.h	1.14	12/08/30 SMI"
...
/*
 * Per-CPU DTrace state buffers cannot be allocated while holding cpu_lock
 * because this does not scale to many CPUs.
 * DTrace must release cpu_lock and dtrace_lock before allocating buffers, and
 * then DTrace must re-acquire these locks afterwards.  These two data
 * structures are used to specify and track the allocated per-CPU buffers,
 * so they can later be copied to dtrace_state after re-acquiring cpu_lock
 * and dtrace_lock.
 */
typedef struct dtrace_buf_alloc {
	int		flags;
	size_t		size;
	dtrace_buffer_t	*bufs;
} dtrace_buf_alloc_t;

typedef struct dtrace_buffers {
	void			*cpus;
	dtrace_buf_alloc_t	buffer;
	dtrace_buf_alloc_t	aggbuffer;
	dtrace_buf_alloc_t	spec_buffer;
	int			nspeculations;
	int			resize;
} dtrace_buffers_t;

/*
 * To avoid holding cpu_lock for a long period of time while destroying
 * DTrace state, DTrace delays the time-consuming operation of freeing the
 * per-CPU buffers until after it can drop cpu_lock.  cpu_lock cannot be
 * released until after dtrace_state has been destroyed.
 * The dtrace_delayed_free structure is used when destroying DTrace state to
 * keep track of the state buffers that need to be freed after DTrace can
 * release cpu_lock.
 */
typedef struct dtrace_delayed_free {
	dtrace_buffer_t *dts_buffer;		/* principal buffer */
	dtrace_buffer_t *dts_aggbuffer;		/* aggregation buffer */
	dtrace_speculation_t *dts_speculations;	/* speculation array */
	int dts_nspeculations;			/* number of speculations */
} dtrace_delayed_free_t;

/*
 * The size must be pointer size aligned (8 bytes).  If the size is not
 * properly aligned, drop it down by the difference.
 */
#define	DTRACE_SIZE_ALIGN(s)	((s) & (~(sizeof (void *) - 1)))
