...
#pragma ident	"@(#)transport.h	1.50	12/06/14 SMI"
...
/*
 * Opaque handle to a group of dma handling information
 */
typedef struct __scsi_dma_info		scsi_dma_info_t;
...
struct scsi_hba_tran {
	...
	/*
	 * The two fields below are for HBA drivers to provide their
	 * expectation of maximal cdb/scb length.
	 */
	size_t			tran_cdb_len;
	size_t			tran_scb_len;

	/*
	 * The fields below are supposed to be provided by HBA drivers if they
	 * need SCSA's assitance of dma pooling.
	 */
	ddi_dma_attr_t		tran_dma_attr_cmd;
	ddi_dma_attr_t		tran_dma_attr_rsp;
	size_t			tran_cmd_len;
	size_t			tran_rsp_len;

	/*
	 * private members for dma pooling for cmd/rsp dma resource allocation
	 */
	scsi_dma_info_t		*tran_cmd_dma_info;
	scsi_dma_info_t		*tran_rsp_dma_info;
...
void		scsi_hba_pkt_comp_from_start(
			struct scsi_pkt	*pkt);
...
dev_info_t	*scsi_hba_iport_find(
			...
			char			*portnm);
/*
 * The interfaces below only exist with SCSA scsi_pkt method-2 (for
 * HBA drivers that use tran_setup_pkt(9E).
 */
caddr_t		scsi_pkt2cmd_dma_vaddr(
				struct scsi_pkt		*pkt);

ddi_acc_handle_t *scsi_pkt2cmd_dma_acc_hdl(
				struct scsi_pkt		*pkt);

ddi_dma_handle_t *scsi_pkt2cmd_dma_handle(
				struct scsi_pkt		*pkt);

ddi_dma_cookie_t *scsi_pkt2cmd_dma_cookie(
				struct scsi_pkt		*pkt);

caddr_t		scsi_pkt2rsp_dma_vaddr(
				struct scsi_pkt		*pkt);

ddi_acc_handle_t *scsi_pkt2rsp_dma_acc_hdl(
				struct scsi_pkt		*pkt);

ddi_dma_handle_t *scsi_pkt2rsp_dma_handle(
				struct scsi_pkt		*pkt);

ddi_dma_cookie_t *scsi_pkt2rsp_dma_cookie(
				struct scsi_pkt		*pkt);
/*
...
#define	SCSI_HBA_HBA		0x10		/* using common ddi_fm_* */

#define	SCSI_HBA_ADDR_SPI	0x20		/* scsi_address in SPI form */
#define	SCSI_HBA_ADDR_COMPLEX	0x40		/* scsi_address is COMPLEX */

#define	SCSI_HBA_TRAN_CMD_DMA	0x80	/* allocate tran_dma_attr_cmd cmd/cdb */
#define	SCSI_HBA_TRAN_RSP_DMA	0x100	/* allocate tran_dma_attr_rsp rsp/scb */
