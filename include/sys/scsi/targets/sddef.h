...
#pragma ident	"@(#)sddef.h	1.172	12/11/07 SMI"
...
/* Should these go somewhere common with CMLB?? XXX */
#define	SD_2TB_BLOCKS		0xffffffff
#define	SD_1TB_BLOCKS		0x7fffffff

#define	SD_EXTVTOC_LIMIT	SD_2TB_BLOCKS
#define	SD_OLDVTOC_LIMIT	SD_1TB_BLOCKS
 ...
 */
struct sd_w_map {
	daddr_t		wm_start;	/* Write start location */
	daddr_t		wm_end;		/* Write end location */
...
struct sd_lun {
	 ...
	 * Otherwise, un_solaris_size represents the capacity corresponding
	 * to VTOC label, or the whole disk if EFI labeled.
	 *
	 ...
	 */
	uint64_t	un_solaris_size;	/* size of Solaris partition */
	...
	uint32_t
	    ...
	    un_f_sync_cache_required	:1,	/* flag to check if */
						...
						/* sent in sdclose */
	    un_f_2TBvtoc_log		:1,	/* whether to log msg */
						/* about vtoc on 2TB disk */
	    ...
	    un_f_pbsize_configured	:1,	/* physical block size is set */
						/* via sd.conf */
	    un_f_reserved		:3;	/* padding */
	...
	sd_label_t	un_label_from_media;	/* type of label on disk */
...
#define	SD_FI_MAX_ERROR 1024
	...
	sd_label_t	un_def_labeltype;	/* default label type */

	/* label type based on which minor nodes were created last */
	sd_label_t	un_last_labeltype;

	sd_label_t	un_cur_labeltype;	/* current label type */

};

/*
 * The sd_minfo_ext32 structure is defined for the convertion
 * between 32-bt data model and 64-bit data model when handle
 * DKIOCGMEDIAINFOEXT request on X86 platform.
 * On SPARC platform, the structure dk_minfo_ext is compiled
 * to the same size (24 bytes) in userland and kernel.
 */
#if defined(_SYSCALL32)
typedef struct sd_minfo_ext32 {
	uchar_t sd_media_type[4];
	uchar_t sd_lbsize[4];
	uchar_t sd_capacity[8];
	uchar_t sd_pbsize[4];
} sd_minfo_ext32_t;
#endif
...
/*
 * Check if the Size of an IO is aligned with the logical block size.
 */
#define	SD_IOSIZE_ALIGN_TGTBLKSZ(un, size)				\
	(!((size) & ((un)->un_tgt_blocksize - 1)))
...
#define	ISDIRECT(un)    (SD_INQUIRY((un))->inq_dtype == DTYPE_DIRECT)
/* disk which has large block size */
#define	IS_LBLKSZ_DISK(un)				\
	(ISDIRECT(un) && ((un)->un_tgt_blocksize > DEV_BSIZE))
...
#define	SD_PKT_ALLOC_NOTALIGNED 		5
...
/* Page Control of Mode SENSE */
#define	SD_MODE_SENSE_PC_CURRENT	0x00
#define	SD_MODE_SENSE_PC_CHANGEABLE 	0x01
#define	SD_MODE_SENSE_PC_DEFAULT	0x10
#define	SD_MODE_SENSE_PC_SAVED		0x11
