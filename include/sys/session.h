...
#pragma ident	"@(#)session.h	1.18	07/05/21 SMI"
 ...
 * p_splock. There are a few deleted member entries in the structure
 * to maintain backward compatibility with a third party software.
 */

typedef struct sess {
	uint_t s_ref; 			/* reference count */
	mode_t pad1;			/* deleted member */
	uid_t pad2;			/* deleted member */
	gid_t pad3;			/* deleted member */
	time_t pad4;			/* deleted member */
	/*
	 * s_dev, s_vp, and s_cred can only be updated while s_lock is held
	...
	int s_cnt;			/* active users of this session */
	int pad5;			/* deleted member */
