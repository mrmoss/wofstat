...
#pragma ident	"@(#)strsubr.h	1.152	11/10/04 SMI"	/* SVr4.0 1.17	*/
...
#define	STZCVMSAFE	0x00100000	/* safe to borrow file (segmapped) */
					/* pages instead of bcopy */
#define	STZCVMUNSAFE	0x00200000	/* unsafe to borrow file pages */
...
/*
 * Each stack (zone with the EXCL flag) has one of these
 */
#ifndef NSTRPHASH
...
struct str_stack {
	...
	struct autopush	*ss_autopush;	/* autopush data array */
	int		ss_nautopush;	/* number of elements in autopush */
	struct autopush **ss_strpcache;	/* autopush hash list */
	int		ss_strpmask;	/* used in hash function */
	struct autopush *ss_strpfreep;	/* autopush freelist */
...
extern int putiocd(mblk_t *, mblk_t *, caddr_t, int, cred_t *);
...
extern mblk_t *mcopyinuio(struct stdata *, uio_t *, ssize_t, ssize_t,
		cred_t *cr, int *);
