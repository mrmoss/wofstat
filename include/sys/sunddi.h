...
#pragma ident	"@(#)sunddi.h	1.192	12/05/11 SMI"
...
#define	DDI_EAGAIN	(-3)	/* not enough resources */
...
#define	DDI_NO_MODUNLOAD	"ddi-no-modunload"
...
/* if set to B_TRUE is DER_MODE is equivalent to DER_PANIC */
...
typedef struct plat_alias_val {
	char *pa_str;
	int pa_flg;
} plat_alias_val_t;

#define	DDI_ALIAS_PRE	0x0001

typedef struct plat_alias {
	char *pali_current;
	uint64_t pali_naliases;
	plat_alias_val_t *pali_aliases;
} plat_alias_t;
...
/*
 * Three boolean flags with regard to device path aliasing.
 *
 * aliases_present: Firmware contains a list of aliases. These
 * will be checked in device path lookups if a lookup fails,
 * since a device path may have been generated in a different
 * configuration.
 *
 * aliases_active: The system is in failover mode (e.g. a CPU
 * module failed and the associated PCI bus was moved). This
 * means that aliases must be checked for instance assignment
 *
 * pre_alias: Aliasing is active (aliases_active will also be set),
 * and there are now active device paths that pointed to different
 * devices in the normal configuration. These aliases must be checked
 * first in instance assignment, to avoid clashes.
 */
extern boolean_t ddi_aliases_present;
extern boolean_t ddi_aliases_active;
extern boolean_t ddi_pre_alias;
...
char *ddi_alias_str(char *alias);
char *ddi_search_aliases(char *path, unsigned how, int (*f)(const char *,
    const char *, void *), void *);
int ddi_is_pre_alias(char *);

#define	DDI_MATCH_ALIAS		0x0001  /* search key is the alias value */
#define	DDI_MATCH_CURR		0x0002  /* search key is the current value */
