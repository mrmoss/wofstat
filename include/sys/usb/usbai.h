...
#pragma ident	"@(#)usbai.h	1.32	12/04/23 SMI"
...
#define	USB_INVALID_STATE	-16	/* invalid slot/endpoint state	  */
...
typedef enum {
	...
	USB_CR_TRANS_ERROR	= 22,	/* timeout/crc/bad PID		*/
	USB_CR_TRB_ERROR	= 23,	/* TRB parameter error		*/
	USB_CR_INCOMPATIBLE_ERR	= 24	/* Incompatible device error	*/
} usb_cr_t;
...
/*
 * usb_cap_descr
 *	The usb device capability descriptor header is defined in USB3.0/9.6.2.
 */
...
#define	USB_CAP_TYPE_USB2_EXT	2
#define	USB_CAP_TYPE_SSUSB	3
#define	USB_CAP_TYPE_CNTER_ID	4

#define	USB_BOS_DESCR_SIZE		5

/*
 * usb_usb2_ext_descr:
 *	usb 2.0 extension descriptor, refer to USB3.0 9.6.2.1
 */
...
} usb_usb2_ext_descr_t;

/*
 * usb_ssdev_cap_descr:
 *	SuperSpeed USB device Capibility descriptor, refer to USB3.0 9.6.2.2
 */
typedef struct usb_ssdev_cap_descr {
	uint8_t		bLength;
	uint8_t		bDescriptorType;
	uint8_t		bDevCapabilityType;
...
} usb_ssdev_cap_descr_t;

/*
 * usb_container_descr:
 *	SuperSpeed USB device Capibility descriptor, refer to USB3.0 9.6.2.3
 * It is used in all the USB 3.0 hubs, and optional for other devices.
 */
typedef struct usb_container_descr {
	uint8_t		bLength;
	uint8_t		bDescriptorType;
	uint8_t		bDevCapabilityType;
	uint8_t		bReserved;
	uint8_t		ContainerID[16];
} usb_container_descr_t;

/*
 * usb_ss_ep_comp_descr:
 *	SuperSpeed endpoint companion descriptor, refer to USB3.0 9.6.7
 */
typedef struct usb_ss_ep_comp_descr {
	uint8_t		bLength;
	uint8_t		bDescriptorType;
	uint8_t		bMaxBurst;
	uint8_t		bmAttributes;
	uint16_t	wBytesPerInterval;
} usb_ss_ep_comp_descr_t;

#define	USB_USB2_EXT_CAP_DESCR_SIZE		7
#define	USB_SSUSB_CAP_DESCR_SIZE		10
#define	USB_CONTAINER_CAP_DESCR_SIZE		20
#define	USB_SS_EP_COMP_DESCR_SIZE		6
...
typedef struct usb_ep_data {
	...
	usb_ss_ep_comp_descr_t	ss_ep_comp_descr; /* superspeed endpoint comp */
} usb_ep_data_t;
...
/*
 * Data structure for USB BOS descriptor
 */
typedef struct usb_bos_data {
	usb_bos_descr_t		bos_descr;	/* parsed bos descr */
	usb_usb2_ext_descr_t	usb2_ext_descr;	/* USB2 extension descr */
	usb_ssdev_cap_descr_t	ssdev_cap_descr; /* SuperSpeed USB cap descr */
	usb_container_descr_t	container_descr; /* container ID descr */
} usb_bos_data_t;
...
/*
 * SuperSpeed endpoint companion descriptor type, refer to USB3.0 spec
 * table 9-5.
 */
#define	USB_DESCR_TYPE_SSUSB_EP_COMPANION	0x30
...
/* USB 3.0 specific requests, refer to USB3.0 spec table 9-4 */
#define	USB_REQ_SET_ISOCH_DELAY		0x30
#define	USB_REQ_SET_SEL			0x31
...
/* USB 3.0 feature selectors, refer to USB3.0 spec table 9-6 */
#define	USB_IF_FUNC_SUSPEND		0x0000
#define	USB_DEV_U1_ENABLE		0x0030
#define	USB_DEV_U2_ENABLE		0x0031
#define	USB_DEV_LTM_ENABLE		0x0032
