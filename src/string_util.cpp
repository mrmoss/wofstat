...
std::string ipv6_prettify(std::string addr)
{
	for(int ii=0;ii<2;++ii)
		addr=replace_all(addr,"00","0");
	if(addr.substr(0,2)=="0:")
	{
		addr.erase(0,1);
		addr[0]=':';
	}
	if(addr.substr(addr.size()-2,2)==":0")
	{
		addr.erase(addr.size()-2,2);
		addr+=':';
	}
	while(true)
	{
		...
		size_t pos=0;
		if((pos=copy.find(":0",0))!=std::string::npos)
			copy.replace(pos,2,":");
		pos=0;
		size_t count=0;
...
std::string u32_to_ipv4(const uint32_t addr)
{
	std::ostringstream ostr;
	ostr<<(uint32_t)((uint8_t*)&addr)[0]<<"."<<
...
std::string u8x16_to_ipv6(const uint8_t addr[16])
{
	std::ostringstream ostr;
